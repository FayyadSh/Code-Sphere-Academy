...
// ------------ Icons ----------------
import { 
  Brush, ChevronDown, ChevronUp, Database, Menu, SearchX, Search,
  SearchCheck, ShoppingCart, Smartphone, TestTubeDiagonal, X } 
from "lucide-react";
...
const Navbar = () => {
  ...
  // ------------ State Variables ----------------
  // State variables to manage the visibility of cart, navbar, dropdown, search bar, and auth dropdown
    ...
    <header className="bg-background-color fixed w-[100vw] dark:bg-background-dark-color transition-all shadow-md z-30">
      <div className="mx-auto flex h-[70px] max-w-screen-xl items-center px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-1 items-center justify-end lg:justify-between">
          ...
          {/* Search input */}
          <SearchInput
            otherClasses={`!w-screen -translate-x-14 md:translate-x-0 md:!w-2/3 xl:!w-1/2 absolute md:right-3 ${showSearchBar ? 'top-[34px] opacity-100' : '-top-64 opacity-0'} transition-opacity duration-300 md:-top-4 h-9 text-xs tracking-widest dark:text-gray-400 absolute rounded-none md:rounded-md placeholder-gray-500 md:pl-4`}            
            iconClasses={`max-md:!-right-56 top-[40px] md:-top-2 opacity-${showSearchBar ? '100' : '0'} transition-all duration-500`}
          />
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleShowSearchBar} >
              {showSearchBar ?
                <SearchX className='text-gray-400 block md:hidden' />
                :
                <SearchCheck className='text-gray-400 block md:hidden' />
              }
            </button>
